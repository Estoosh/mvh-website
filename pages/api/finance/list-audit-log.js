...
// Reads only the Audit_Log table (new Finance table) — does not touch
// Guides or Tours.
...
export default async function handler(req, res) {
  ...
  if (!FINANCE_TABLES.AUDIT_LOG) {
    return res.status(500).json({ error: 'audit_log_table_not_configured' })
  }
  ...
  try {
    ...
    const response = await fetch(
      `${AIRTABLE_API_URL}/${baseId}/${FINANCE_TABLES.AUDIT_LOG}?${params.toString()}`,
