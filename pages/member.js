...
export default function Member() {
  ...
  const [newFromFollowed, setNewFromFollowed] = useState(null)
  ...
  useEffect(function() {
    ...
    fetch('/api/get-saved-tours?clerk_id=' + user.id)
      ...
      .catch(() => setSavedTours([]))
    fetch('/api/get-new-from-followed?clerk_id=' + user.id)
      .then((r) => r.json())
      .then((data) => setNewFromFollowed(data.tours || []))
      .catch(() => setNewFromFollowed([]))
  ...
  return (
    <div style={{ fontFamily: 'Heebo, Arial, sans-serif', background: CREAM, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, maxWidth: 900, margin: '0 auto', padding: '48px 24px 64px', width: '100%' }}>
        ...
        {/* New from people you follow — highest-value section, shown first */}
        {newFromFollowed && newFromFollowed.length > 0 && (
          <div style={{ background: '#fff', borderRadius: 18, border: '1px solid #EDE7DF', padding: 28, marginBottom: 24 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>יש משהו חדש ממי שאתם עוקבים אחריהם</h2>
            <p style={{ fontSize: 13, color: '#6B6B6B', marginBottom: 18 }}>מדריכים שאתם עוקבים אחריהם העלו סיורים חדשים לאחרונה</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {newFromFollowed.map(function(tour) {
                const images = tour.Tour_Images ? tour.Tour_Images.split('|').filter(Boolean) : []
                const thumb = images[0] || null
                return (
                  <Link key={tour.id} href={'/tours/' + tour.id} style={{ textDecoration: 'none', color: 'inherit', display: 'grid', gridTemplateColumns: '90px 1fr', background: CREAM, borderRadius: 12, overflow: 'hidden' }}>
                    <div style={{ height: 80, background: '#e8e0d8' }}>
                      {thumb
                    ...
                    <div style={{ padding: '10px 16px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <p style={{ fontSize: 11, color: BROWN, fontWeight: 700, marginBottom: 2 }}>חדש מ-{tour.Guide_Name}</p>
                      <p style={{ fontSize: 14, fontWeight: 700 }}>{tour.Tour_Title}</p>
                    </div>
                  </Link>
                )
              })}
