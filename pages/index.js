    ...
    <div>
      ...
      <div style={{ background: '#0A0A0A', padding: '72px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <h1 style={{ fontSize: 56, fontWeight: 800, color: '#ffffff', lineHeight: 1.1, marginBottom: 20 }}>MvH</h1>
          ...
          {!done ? (
            <form onSubmit={function(e) { e.preventDefault(); if (email) setDone(true) }}
              ...
              <input type="email" value={email} onChange={function(e) { setEmail(e.target.value) }} required
                style={{ flex: 1, minWidth: 200, padding: '14px 18px', borderRadius: 6, border: '1px solid #333', background: '#111', color: '#fff', fontSize: 15, outline: 'none' }} />
              <button type="submit" style={{ background: '#C4922A', color: '#fff', padding: '14px 28px', borderRadius: 6, fontSize: 15, fontWeight: 700 }}>GO</button>
            </form>
          ) : (
...
export async function getServerSideProps() {
  try {
    ...
    const cities = [...new Set(tours.map(function(t) { return t.Cities_Tags }).filter(Boolean))].sort()
    return { props: { tours, cities } }
  } catch(e) {
    return { props: { tours: [], cities: [] } }
  }
}
